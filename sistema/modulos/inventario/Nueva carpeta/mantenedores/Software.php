...
//aca comenzamos la creacion del select para cargar y editar
{
	...
	if($fila=mysqli_fetch_row(mysqli_query($link,$selectSoftware)))
	{
...
if($idequipo!="" && $nbsoftware!="" && $tiposoftware!="" && $arquitectura !="" && $empresa!="" && $activacion!="" && $version!="" && $pagina !="" && $opcion!="")
...
//********************************************************************************************************
	...
	{	
        ...
        $recurso = mysqli_query($link,$consultarSoftware);
        if($recurso)
        {
		...
		$recurso = mysqli_query($link,$crearSoftware);
...
//*****************************************************************************************************
	...
	SOFT_ANOVERS='$anoversion',	SOF_FCREACION='$fcreacion',SOFT_VERSION='$version',SOFT_WEBSOFT='$pagina'
		...
		if(mysqli_query($link,$actualizarSoftware))
		//inicio de la consulta del recurso != null
		{
...
$vacio = "";
$selected = "";
if($idequipo=="")$vacio="selected='selected'";
?>
...
$recurso = mysqli_query($link,$ubicacion);

if($recurso){
	while($fila=mysqli_fetch_array($recurso,MYSQLI_NUM))
	{
		if($idequipo == $fila[0])$selected ="selected='selected'";
...
$cantidadRegistros = mysqli_num_rows(mysqli_query($link,"select SOF_IDSOFTW from software"));

$paginar_resultados = "select software.SOF_IDSOFTW, software.SOF_NBSOFTW, software.SOFT_EMPRESA from software  order by software.SOF_IDSOFTW  limit " . (( $paginadorZebra->get_page() - 1 ) * $cant_pagina ) . ",  " . $cant_pagina;
