...
//aca comenzamos la creacion del select para cargar y editar
{
	...
	if($fila=mysqli_fetch_row(mysqli_query(Conexion(),$selectRed)))
	{
		$id = $fila[0];
		$tipo=$fila[1];
		$ubicacion =$fila[2];
...
//*****************************************************************************************************
	...
	{
		$actualizarRed= "UPDATE `sisgen`.`red` SET `TIP_IDTIPO` = '$tipo',
        ...
        `RED_COSTO` = '$costo' WHERE `red`.`RED_IDRED` =$id;"; 	
    ...
    <div class="row">
        ...
        <div class="col-xs-8" id="formularios">
            ...
            <table width="500" align="center" class="table-hover" style="font-size:12px;">
            	<th colspan="4" class="text-center">mantenedor de equipos de red</th>
                <tr>
...
$paginar_resultados = "select * from red order by red.RED_IDRED ASC limit " . (( $paginadorZebra->get_page() - 1 ) * $cant_pagina ) . ",  " . $cant_pagina;
