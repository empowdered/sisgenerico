...
$faanterior =""; //opcional, tomara el valor anterior de la fecha ingresada, 
//luego asumira esa variable como nueva, en caso de que venga vacia, asumira el valor de esta misma variable.
...
$fila=mysqli_fetch_row(mysqli_query(Conexion(),$selectImpresora));
if($fila)
...
{
$actualizarImpresora = "UPDATE `sisgen`.`impresion` SET `TIP_IDTIPO` = '$tipo',`UB_IDUBICACION` = '$ubicacion', `IM_SERIEIMPRESION` = '$serieequipo', `IM_MARCA` = '$marca', `IM_MODELO` = '$modelo', `IM_TIPOIMPRESION` = '$tipoimpresora', `IM_DESCRIPCION` = '$descripcion', IM_ESTADOIMP='$estado', `IM_COSTO` = '$costo' WHERE `impresion`.`IM_IDIMPRESION` = '$id'";
...
<td><input type="text" name="fingreso" value="" size="10" id="fingreso" readonly="readonly"/></td>
<td>fecha anterior:</td>
<td><input type="text" name="faanterior" value="<?=$faanterior?>" size="10" id="fingreso" readonly="readonly"/></td>
</tr>        
<tr>
...
$paginar_resultados = "select impresion.*,marca.MAR_NBMARCA from impresion,marca where
impresion.IM_MARCA = marca.MAR_IDMARCA
order by IM_IDIMPRESION  limit " . (( $paginadorZebra->get_page() - 1 ) * $cant_pagina ) . ",  " . $cant_pagina;
...
while($fila=mysqli_fetch_array($recurso,MYSQLI_NUM))
{
echo "<tr><td>".$fila[0]."</td><td>".$fila[12]."<td title='".$fila[7]."'>".substr($fila[7],0,10)."...</td>";
