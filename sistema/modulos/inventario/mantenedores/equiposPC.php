...
$resultado = mysqli_query(Conexion(),"select count(equipo.EQ_IDEQUIPO) FROM equipo");
$cantElementos = mysqli_fetch_row($resultado);
$recurso = NULL;
//*******************estas 2 son las variables para cargar el tipo
$cargar = "";
$id = "";
//*******************seteamos las variables o las declaramos en su efecto
$ubicacion = "";
$tipo ="";
$nroequipo = "";
$serieequipo = "";
...
if($fingreso==""){$fingreso = $fechaGuardada;}
...
//sino resulta  se envia un mensaje de error, se libera recursos 
{
mysqli_free_result($recurso);
...
{
$recurso = NULL;
$actualizarEquipo = "UPDATE equipo SET UB_IDUBICACION='$ubicacion',EQ_IDTIPO='$tipo', EQ_NROEQUIPO='$nroequipo',
EQ_SERIEEQUIPO='$serieequipo',EQ_MARCA='$marcaequipo',EQ_MODELO='$modeloequipo',EQ_RAM='$ramequipo', 
EQ_DISCO='$discoequipo',EQ_MAC='$macequipo',EQ_IP='$ipequipo',EQ_PTAENLACE='$enlaceequipo', 
EQ_MASCARA='$mascaraequipo',EQ_PROCESADOR='$procesador',EQ_ESTADO='$estado',EQ_FINGRESO='$fingreso',
EQ_FCREACION ='$fcreacion',EQ_COSTO='$costoequipo' WHERE EQ_IDEQUIPO=" . $id;
echo "<br>".$actualizarEquipo;
    ...
    <div class="row">
        ...
        <input type="hidden" value="<?=$id?>" name="id" />
        	<table width="600" align="center" class="table-hover" style="font-size:12px;">
                    <tr> <td colspan="4" class="text-center">mantenedor de equipos pc</td></tr>
<tr>
<td colspan="1" style="text-align:right;">ubicacion:</td>
<?php
$vacio="";
$selected ="";
if($ubicacion=="")
...
$ubicacionSQL = "select * FROM ubicacion order by ubicacion.UB_IDUBICACION ASC";
$recurso = mysqli_query(Conexion(),$ubicacionSQL);
?>
...
$marca = "";
$vacio = "";
if($marcaequipo==""){$vacio="selected='selected'";}
?>
...
<tr>
<td>Dir IP:</td>
<td><input name="ipequipo" type="text" id="ipequipo" value="<?=$ipequipo?>" size="25" onblur="validateIp('ipequipo');" /></td>
<td>&nbsp;Pta Enlace:</td>
<td><input name="enlaceequipo" type="text" id="enlaceequipo" value="<?=$enlaceequipo?>" size="15" onblur="validateIp('ipequipo');"/></td>
</tr>
<tr>
<td>Mascara subred:</td>
<td><input name="mascaraequipo" type="text" id="mascaraequipo" value="<?=$mascaraequipo?>" size="25" onblur="validateIp('ipequipo');"/></td>
<td>&nbsp;Procesador:</td>
...
<tr>
<td>
 Fecha ingreso:<span class="glyphicon glyphicon-calendar" onclick="javascript:NewCssCal('fingreso','yyyyMMdd')" style="cursor:pointer"></span></td>
<td>
<input type="text" name="fingreso"  id="fingreso" value="" size="25" />
</td>
<td>Fecha anterior:</td>
<td>
 <input type="text" name="fechaGuardada" id="fechaGuardada" value="<?=$fechaGuardada?>" size="15"/>
</td>
</tr>
<tr>
...
$paginar_resultados = "select equipo.*,marca.MAR_NBMARCA from  equipo,marca "
        . "where equipo.EQ_MARCA = marca.MAR_IDMARCA order by  equipo.EQ_IDEQUIPO  asc  limit " . 
        (( $paginadorZebra->get_page() - 1 ) * $cant_pagina ) . ",  " . $cant_pagina;
...
while($fila=mysqli_fetch_array($recurso,MYSQLI_NUM))
{
echo "<tr><td>".$fila[0]."</td><td>".$fila[3]."<td>".$fila[9]."</td><td>".$fila[4]."</td><td>".$fila[18]."</td>";
