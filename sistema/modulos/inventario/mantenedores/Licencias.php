...
if($fingreso==""){$fingreso=$fanterior;}
...
$recurso = mysqli_query(Conexion(),$crearLicencia);
if($recurso!=NULL)
...
{
$actualizarLicencia = "update licencia set `LIC_NROLICENCIA` = '$nrolicencia',`LIC_FCOMPRALIC` ='$fcompra', 
`LIC_TIPOLICENCIA`='$tipolic',`LIC_FVENCIMIENTO`='$fvencimiento',`LIC_FINGRESO`='$fingreso',`LIC_SERIELICENCIA`='$seriel', 
`LIC_USUARIOLIC`='$usuario',`LIC_CORREOASOCIADO` ='$correo' where licencia.LIC_IDLICENCIA='$id'";
$recurso = mysqli_query(Conexion(),$actualizarLicencia);
if($recurso!=NULL)
//inicio de la consulta del recurso != null
{
...
<input type="hidden" name="id" id="id" value="<?=$id?>" />
       <table width="600" align="center" class="table-hover" style="font-size:12px; height:100px;">
...
software.SOF_IDSOFTW from equipo,software where equipo.EQ_IDEQUIPO = software.EQ_IDEQUIPO
and software.SOF_IDSOFTW  not in(select licencia.SOF_IDSOFTW from licencia)and software.SOFT_ACTIVADO='activo'";
$recurso = mysqli_query(Conexion(),$nbSoftware);
?>
...
$cantidadRegistros = mysqli_num_rows(mysqli_query(Conexion(),"select licencia.LIC_IDLICENCIA from licencia"));
