...
$fila=mysqli_fetch_row(mysqli_query(Conexion(),$selectPeriferico));
if($fila)
{
$id = $fila[0];
$serieequipo = $fila[1];
...
$tipo = "select tipo.TIP_IDTIPO, tipo.TIP_NBTIPO, tipo.TIPO_DESCTIPO FROM tipo";
$recurso = mysqli_query(Conexion(),$tipo);
?>
<select name="tipoequipo" id="tipoequipo" size="1">
...
while($fila=mysqli_fetch_array($recurso,MYSQLI_NUM))
{
if($tipoequipo==$fila[0])
{
echo "<option value='".$fila[0]."' selected='selected'>".$fila[1]."-".substr($fila[2], 0, 4)."</option>";	
}
else
{
echo "<option value='".$fila[0]."'>".$fila[1]."-".substr($fila[2], 0, 4)."</option>";	
}
}
}
?>
...
<tr>
<td>estado:</td>
<td><select name="estadoperif" id="estadoperif">
<option value="" >Seleccionar</option>
...
$paginar_resultados = "select periferico.*,marca.MAR_NBMARCA,tipo.TIP_NBTIPO from periferico, marca, tipo
where periferico.PER_MARCAPERIFERICO = marca.MAR_IDMARCA AND
periferico.TIP_IDTIPO = tipo.TIP_IDTIPO
order by PER_IDPERIFERICO  limit " . (( $paginadorZebra->get_page() - 1 ) * $cant_pagina ) . ",  " . $cant_pagina;
...
while($fila=mysqli_fetch_array($recurso,MYSQLI_NUM))
{
echo "<tr><td>".$fila[0]."</td><td>".$fila[10]."<td>".$fila[4]."</td><td>".$fila[11]."</td>";
